...
def parse_crew_eta(eta_str, ref_date):
    if pd.isna(eta_str) or eta_str == 'N/A' or eta_str == "": 
        return pd.NaT
    try:
        if '{' in str(eta_str):
...
def run_training():
    df = pd.read_csv("FINAL_COMPLETED_DATASET.csv", sep=None, engine='python')
    ...
    # final comparison 
    print(f"\nFinal Performance Comparison ({len(df_clean)} Voyages)")
    ...
    comparison_data = {
        "Model": ["Human (AIS ETA)", "Random Forest", "XGBoost"],
        "MAE_Hours": [round(human_mae, 2), round(rf_mae, 2), round(xgb_mae, 2)],
        "R2_Score": [0.0000, round(rf_r2, 4), round(xgb_r2, 4)],
        "Accuracy_Gain_Percent": [0.0, round(rf_gain, 2), round(xgb_gain, 2)]
    }
    pd.DataFrame(comparison_data).to_csv("performance_comparison_summary.csv", index=False)

    importances_df = pd.DataFrame({
        "Feature": features,
        "Random_Forest_Weight": rf.feature_importances_,
        "XGBoost_Weight": xgb.feature_importances_
    }).sort_values(by="Random_Forest_Weight", ascending=False)
    importances_df.to_csv("feature_importance_weights.csv", index=False)

    print("Files Saved:")
    print("- performance_comparison_summary.csv")
    print("- feature_importance_weights.csv")
